...
try
    ...
    % Calcul de la nouvelle fréquence en fonction de la conductivité optimale
    N_Freq_2 = sig_freq/c_res_2

    % Réouverture du port série pour la nouvelle configuration
    fopen(s);

    % Mise à jour de la fréquence
    fprintf(s, ['FREQUE,', num2str(N_Freq_2)]);
    fprintf(s, '*TRG');
    pause(2);
    fprintf(s, 'LCR?');
    pause(2);
    ...
    % Nouvelle fonction d'optimisation
    f2 = @(c1) abs(Z_integral(coil, N_Freq_2/1000, t1, l0, [c1, 0], mu, cup) - Z_mes_2)^2;
    fun_2 = @(c1) f2(c1);

    % Optimisation pour la nouvelle fréquence
    c_res_3 = fminsearch(fun_2, c1_0, options)
